...
#ifndef NW_GRAPH_PARTITIONED_BUILD_HPP
#define NW_GRAPH_PARTITIONED_BUILD_HPP
...
namespace nw::graph {
  ...
  //  default_execution_policy> void sort_by(edge_list_t& el, ExecutionPolicy&& policy = {}) {
  //    std::sort(std::execution::seq, el.begin(), el.end(), [](const auto& a, const auto& b) ->
  //    bool
  //              { return (std::get<idx>(a) < std::get<idx>(b)); });
  //  }
  //
  //  template <int idx, edge_list_graph edge_list_t, class ExecutionPolicy =
  //  default_execution_policy> void stable_sort_by(edge_list_t& el, ExecutionPolicy&& policy = {})
  //  {
  ...
  //
  //  template <int idx, edge_list_graph edge_list_t, class ExecutionPolicy =
  //  default_execution_policy> void lexical_sort_by(edge_list_t& el, ExecutionPolicy&& policy = {})
  //  {
  ...
  //
  //  template <int idx, edge_list_graph edge_list_t, class ExecutionPolicy =
  //  default_execution_policy> void lexical_stable_sort_by(edge_list_t& el, ExecutionPolicy&&
  //  policy = {}) {
  //
  //    const int jdx = (idx + 1) % 2;
  //
  ...
  //                     [](const auto& a, const auto& b) -> bool
  //                     {
  //                       return std::tie(std::get<idx>(a), std::get<jdx>(a)) <
  //                         std::tie(std::get<idx>(b), std::get<jdx>(b));
  //                     });
  //  }
  //
  //
  ...
  // #if NWGRAPH_HAVE_TBB
  //    tbb::parallel_for(tbb::blocked_range<std::uint64_t>(0ul, perm.size()),
  //                      [&](auto&& r)
  //                      {
  ...
  //    hpx::experimental::for_loop(hpx::execution::par, 0ul, perm.size(),
  //                                [&](auto&& i) { vec2[i] = vec1[perm[i]]; });
  // #endif
  //  }
  //
  //  template <edge_list_graph edge_list_t, adjacency_list_graph adjacency_t, class Perm, size_t...
  //  Is> void permute_helper(edge_list_t& el, adjacency_t& cs, std::index_sequence<Is...> is,
  //                      const Perm& perm) {
  //    (...,
  //     (permute(std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(el)),
  //              std::get<Is + 1>(cs.to_be_indexed_), perm)));
  //  }
  //
  //  template <edge_list_graph edge_list_t, adjacency_list_graph adjacency_t, class Perm, size_t...
  //  Is> void permute_helper_all(edge_list_t& el, adjacency_t& cs, std::index_sequence<Is...> is,
  //                          const Perm& perm) {
  //    (...,
  //     (permute(std::get<Is + 1>(dynamic_cast<typename edge_list_t::base&>(el)),
  //              std::get<Is>(cs.to_be_indexed_), perm)));
  //  }
  //
  //
  //  template <edge_list_graph edge_list_t, adjacency_list_graph adjacency_t, class T, class Perm,
  //            size_t... Is>
  //  void permute_helper(edge_list_t& el, adjacency_t& cs, std::index_sequence<Is...> is, T& Tmp,
  //                      Perm& perm) {
  //    (...,
  //     (permute(std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(Tmp)),
  //              std::get<Is + 1>(cs.to_be_indexed_), perm)));
  //  }

  template <edge_list_graph edge_list_t, adjacency_list_graph adjacency_t,
            class ExecutionPolicy = default_execution_policy, size_t... Is>
  void partitioned_fill_helper(edge_list_t& el, adjacency_t& cs, std::index_sequence<Is...> is,
                               ExecutionPolicy&& policy = {}) {
    (...,
     (hpx::copy(policy, std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(el)).begin(),
                std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(el)).end(),
                std::get<Is + 1>(cs.to_be_indexed_).begin())));
  }

  template <edge_list_graph edge_list_t, adjacency_list_graph adjacency_t,
            class ExecutionPolicy = default_execution_policy, size_t... Is>
  void partitioned_copy_helper(edge_list_t& el, adjacency_t& cs, std::index_sequence<Is...> is,
                               size_t offset, ExecutionPolicy&& policy = {}) {
    (...,
     (hpx::copy(policy, std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(el)).begin(),
                std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(el)).end(),
                std::get<Is + 1>(cs.to_be_indexed_).begin() + offset)));
  }

  template <edge_list_graph edge_list_t, adjacency_list_graph adjacency_t, class T,
            class ExecutionPolicy = default_execution_policy, size_t... Is>
  void partitioned_fill_helper_tmp(edge_list_t& el, adjacency_t& cs, std::index_sequence<Is...> is,
                                   T& Tmp, ExecutionPolicy&& policy = {}) {
    (...,
     (hpx::copy(policy, std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(Tmp)).begin(),
                std::get<Is + 2>(dynamic_cast<typename edge_list_t::base&>(Tmp)).end(),
                std::get<Is + 1>(cs.to_be_indexed_).begin())));
  }

  /**
   * @brief This function fills an adjacency list graph structure from edges contained in a
   * directed edge list.
   *
   * If edge (u,v) is the edge list, the edge (u,v) will be inserted into the adjacency list --
   * that is, v will be inserted into the neighborhood of u.
   *
   * If the edges in the edge list have properties, those properties will be copied to the
   * adjacency list.
   *
   ...
   * @param policy The parallel execution policy to use in calls to standard library algorithms
   * called by this function.
   */
  template <int idx, edge_list_graph edge_list_t, adjacency_list_graph adjacency_t, class Int,
            class ExecutionPolicy = default_execution_policy>
  void partitioned_fill_directed(edge_list_t& el, Int N, adjacency_t& cs,
                                 ExecutionPolicy&& policy = {}) {
    ...
    // Better yet
    int const kdx = (idx + 1) % 2;
    std::copy(std::get<kdx>(dynamic_cast<typename edge_list_t::base&>(el)).begin(),
              std::get<kdx>(dynamic_cast<typename edge_list_t::base&>(el)).end(),
              std::get<0>(cs.to_be_indexed_).begin());

    // Copy properties
    if constexpr (std::tuple_size_v<typename edge_list_t::attributes_t> > 0) {
      partitioned_fill_helper(
        el, cs,
        std::make_integer_sequence<size_t, std::tuple_size_v<typename edge_list_t::attributes_t>>(),
        policy);
    }

    // which is faster?
    std::vector<nw::graph::vertex_id_t<adjacency_t>> tmp(
      std::get<idx>(dynamic_cast<typename edge_list_t::base&>(el)));
  ...
  template <int idx, edge_list_graph edge_list_t, class Int, adjacency_list_graph adjacency_t,
            class ExecutionPolicy = default_execution_policy>
  void partitioned_fill_undirected(edge_list_t& el, Int N, adjacency_t& cs,
                                   ExecutionPolicy&& policy = {}) {
    ...
    hpx::copy(policy, std::get<kdx>(el).begin(), std::get<kdx>(el).end(),
              std::get<0>(cs.to_be_indexed_).begin());
    hpx::copy(policy, std::get<idx>(el).begin(), std::get<idx>(el).end(),
              std::get<0>(cs.to_be_indexed_).begin() + el.size());

    if constexpr (std::tuple_size_v<typename edge_list_t::attributes_t> > 0) {
      partitioned_copy_helper(
        el, cs,
        std::make_integer_sequence<size_t, std::tuple_size_v<typename edge_list_t::attributes_t>>(),
        0, policy);
    }
    ...
    auto a = make_zipped(Tmp, cs.to_be_indexed_);
    hpx::sort(policy, a.begin(), a.end(),
              [](auto&& a, auto&& b) { return std::get<0>(a) < std::get<0>(b); });
  }

  template <int idx, edge_list_graph edge_list_t, adjacency_list_graph adjacency_t,
            class ExecutionPolicy = default_execution_policy>
  auto partitioned_fill(edge_list_t& el, adjacency_t& cs, bool sort_adjacency = false,
                        ExecutionPolicy&& policy = {}) {
    if constexpr (edge_list_t::edge_directedness == nw::graph::directedness::directed) {
      partitioned_fill_directed<idx>(el, num_vertices(el), cs, policy);
    }
    else { // undirected -- this cannot be a bipartite graph
      partitioned_fill_undirected<idx>(el, num_vertices(el), cs, policy);
    }
    //if (sort_adjacency) {
  ...
  template <int idx, edge_list_graph edge_list_t, adjacency_list_graph adjacency_t,
            class ExecutionPolicy = default_execution_policy>
  auto partitioned_fill(edge_list_t& el, adjacency_t& cs, directedness dir,
  ...
  //  template <int idx, edge_list_graph bi_edge_list_t, adjacency_list_graph biadjacency_t,
  //            class ExecutionPolicy = default_execution_policy>
  //  auto fill_biadjacency(bi_edge_list_t& el, biadjacency_t& cs, bool sort_adjacency = false,
  //                        ExecutionPolicy&& policy = {}) {
  ...
  //  }
  //
  //  template <int idx, edge_list_graph edge_list_t, succession cessor = succession::predecessor,
  //            class ExecutionPolicy = default_execution_policy>
  //  void swap_to_triangular(edge_list_t& el, ExecutionPolicy&& policy = {}) {
  //
  // #ifdef NWGRAPH_HAVE_HPX
  //    if constexpr ((idx == 0 && cessor == succession::predecessor) ||
  //                  (idx == 1 && cessor == succession::successor)) {
  //      hpx::for_each(policy, el.begin(), el.end(),
  //                    [](auto&& f)
  //                    {
  ...
  //                    });
  //    }
  //    else if constexpr ((idx == 0 && cessor == succession::successor) ||
  //                       (idx == 1 && cessor == succession::predecessor)) {
  //      hpx::for_each(policy, el.begin(), el.end(),
  //                    [](auto&& f)
  //                    {
  ...
  //    if constexpr ((idx == 0 && cessor == succession::predecessor) ||
  //                  (idx == 1 && cessor == succession::successor)) {
  //      std::for_each(policy, el.begin(), el.end(),
  //                    [](auto&& f)
  //                    {
  //                      if (std::get<0>(f) < std::get<1>(f)) {
  //                        std::swap(std::get<0>(f), std::get<1>(f));
  ...
  // #ifdef NWGRAPH_HAVE_HPX
  //    auto past_the_end =
  //      hpx::unique(policy, el.begin(), el.end(), [](auto&& x, auto&& y)
  //                  { return std::get<0>(x) == std::get<0>(y) && std::get<1>(x) == std::get<1>(y);
  //                  });
  // #else
  //    auto past_the_end =
  //      std::unique(policy, el.begin(), el.end(), [](auto&& x, auto&& y)
  //                  { return std::get<0>(x) == std::get<0>(y) && std::get<1>(x) == std::get<1>(y);
  //                  });
  // #endif
  //
  ...
  //    auto past_the_end =
  // #ifdef NWGRAPH_HAVE_HPX
  //      hpx::remove_if(/*std::execution::par_unseq,*/ el.begin(), el.end(),
  //                     [](auto&& x) { return std::get<0>(x) == std::get<1>(x); });
  //    // el.erase(past_the_end, el.end());
  // #else
  //      std::remove_if(/*std::execution::par_unseq,*/ el.begin(), el.end(),
  //                     [](auto&& x) { return std::get<0>(x) == std::get<1>(x); });
  //    // el.erase(past_the_end, el.end());
  // #endif
  //    el.resize(past_the_end - el.begin());
  ...
  // #if NWGRAPH_HAVE_TBB
  //    tbb::parallel_for(tbb::blocked_range<std::uint64_t>(0ul, degree_v.size()),
  //                      [&](auto&& r)
  //                      {
  //                        for (auto i = r.begin(), e = r.end(); i != e; ++i) {
  ...
  //                      });
  // #elif NWGRAPH_HAVE_HPX
  //    hpx::experimental::for_loop(hpx::execution::par, 0ul, degree_v.size(),
  //                                [&](auto&& i) { degree_v[i] = degree(graph[i]); });
  // #endif
  //    return degree_v;
  //  }
  //
  //
  //  template <int d_idx = 0, edge_list_graph edge_list_t,
  //            class ExecutionPolicy = default_execution_policy>
  ...
  //
  //  // for bipartite graph
  //  template <int d_idx, edge_list_graph edge_list_t,
  //            class ExecutionPolicy = default_execution_policy>
  ...
  //  }
  //
  //  template <int idx = 0, edge_list_graph edge_list_t, class Vector,
  //            class ExecutionPolicy = default_execution_policy>
  //  auto perm_by_degree(edge_list_t& el, const Vector& degree, std::string direction =
  //  "ascending",
  //                      ExecutionPolicy&& policy = {}) {
  //
  ...
  //
  //  /**
  //   * @brief This function relabels edge list of unipartite graph. It will relabel both
  //   endpoints.
  //   *
  ...
  //   */
  //  template <edge_list_graph edge_list_t, class Vector,
  //            class ExecutionPolicy = default_execution_policy>
  ...
  // #if NWGRAPH_HAVE_TBB
  //    tbb::parallel_for(tbb::blocked_range<std::uint64_t>(0ul, iperm.size()),
  //                      [&](auto&& r)
  //                      {
  //                        for (auto i = r.begin(), e = r.end(); i != e; ++i) {
  //                          iperm[perm[i]] = i;
  ...
  //                  });
  // #elif NWGRAPH_HAVE_HPX
  //    hpx::experimental::for_loop(hpx::execution::par, 0ul, iperm.size(),
  //                                [&](auto&& i) { iperm[perm[i]] = i; });
  //    hpx::for_each(policy, el.begin(), el.end(),
  //                  [&](auto&& x)
  //                  {
  //                    std::get<0>(x) = iperm[std::get<0>(x)];
  ...
  //   * @return the new IDs of the vertices after permutation
  //   */
  //  template <int idx, edge_list_graph edge_list_t, class Vector,
  //            class ExecutionPolicy = default_execution_policy>
  //    requires(false == is_unipartite<typename edge_list_t::bipartite_graph_base>::value)
  //  auto relabel(edge_list_t& el, const Vector& perm, ExecutionPolicy&& policy = {}) {
  //    std::vector<typename edge_list_t::vertex_id_type> iperm(perm.size());
  ...
  // #endif
  //    std::for_each(policy, el.begin(), el.end(),
  //                  [&](auto&& x) { std::get<idx>(x) = iperm[std::get<idx>(x)]; });
  //    return iperm;
  //  }
  //
  //  /**
  //   * @brief This relabel function for edge list handles unipartite graph. It will relabel both
  //   * endpoints.
  //   *
  //   * @tparam edge_list_t, edge list type
  ...
  //    requires(is_unipartite<typename edge_list_t::unipartite_graph_base>::value)
  //  void relabel_by_degree(edge_list_t& el, std::string direction = "ascending",
  //                         const Vector& degree = std::vector<int>(0)) {
  //
  //    std::vector<typename edge_list_t::vertex_id_type> perm = degree.size() == 0
  //      ? perm_by_degree<0>(el, direction)
  //      : perm_by_degree<0>(el, degree, direction);
  //
  //    relabel(el, perm);
  //  }
  //
  ...
  //    requires(is_unipartite<typename edge_list_t::unipartite_graph_base>::value)
  //  auto relabel_by_degree(edge_list_t& el, std::string direction = "ascending",
  //                         const Vector& degree = std::vector<int>(0)) {
  //
  //    std::vector<typename edge_list_t::vertex_id_type> perm = degree.size() == 0
  //      ? perm_by_degree<idx>(el, direction)
  //      : perm_by_degree<idx>(el, degree, direction);
  //    if constexpr (true ==
  //                  is_unipartite<typename edge_list_t::unipartite_graph_base>::value) { //
  //                  Compress
  //                                                                                       // idx
  ...
  //
  //  /**
  //   * Make an edge list without properties from original data, e.g., vector<tuple<size_t,
  //   size_t>>
  //   */
  ...
  //
  //    for (Edge&& e : edges) {
  //      std::apply([&](auto&& u, auto&& v, auto... props_)
  //                 { index_edges.push_back(std::make_tuple(map[u], map[v])); }, e);
  //    }
  //
  //    return index_edges;
  //  }
  //
  //  /**
  //   * Make an edge list with properties copied from original data, e.g., vector<tuple<size_t,
  //   * size_t, props...>>
  //   */
  //  template <class M, std::ranges::random_access_range E,
  ...
  //    for (auto&& e : edges) {
  //      std::apply([&](auto&& u, auto&& v, auto... props_)
  //                 { index_edges.push_back(std::make_tuple(map[u], map[v], props_...)); }, e);
  //    }
  //
  //    return index_edges;
  //  }
  //
  //  /**
  //   * Make an edge list indexing back to the original data, e.g., vector<tuple<size_t, size_t,
  //   * size_t>>
  //   */
  //  template <class I = std::vector<std::tuple<size_t, size_t, size_t>>, class M,
  ...
  //            adjacency_list_graph Graph = std::vector<std::vector<size_t>>>
  ...
  //            adjacency_list_graph Graph = std::vector<std::vector<
  //              decltype(std::tuple_cat(std::make_tuple(size_t{}), props(*(begin(E{})))))>>>
  //  auto make_property_graph(const V& vertices, const E& edges, bool directed = true,
  //                           size_t idx = 0) {
  //
  //    auto vertex_map = make_index_map(vertices);
  ...
  //   */
  //  template <class I = std::vector<std::tuple<size_t, size_t>>, std::ranges::random_access_range
  //  V,
  //            std::ranges::random_access_range E>
  ...
  //  }
  //
  //  template <std::ranges::random_access_range V1, std::ranges::random_access_range V2,
  //            std::ranges::random_access_range E,
  //            adjacency_list_graph Graph = std::vector<std::vector<
  //              decltype(std::tuple_cat(std::make_tuple(size_t{}), props(*(begin(E{})))))>>>
  //  auto make_plain_bipartite_graph(const V1& left_vertices, const V2& right_vertices, const E&
  //  edges,
  //                                  size_t idx = 0) {
  //
  ...
  //
  //  template <std::ranges::random_access_range V1, std::ranges::random_access_range V2,
  //            std::ranges::random_access_range E, class Graph = std::vector<std::vector<size_t>>>
  //  auto make_plain_bipartite_graphs(const V1& left_vertices, const V2& right_vertices,
  //                                   const E& edges) {
  //
  //    auto index_edges = data_to_graph_edge_list<>(left_vertices, right_vertices, edges);
  //
  ...
  //  }
  //
  //  template <size_t idx = 0, class Graph = std::vector<std::vector<size_t>>,
...
#endif // NW_GRAPH_PARTITIONED_BUILD_HPP
