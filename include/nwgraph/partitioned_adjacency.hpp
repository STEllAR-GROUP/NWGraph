...
using compressed = partitioned_index_compressed<default_index_t, default_vertex_id_type, Attributes...>;
  ...
  class partitioned_index_adjacency
    ...
    partitioned_index_adjacency(partitioned_index_adjacency const& rhs, hpx::take_ref)
      : unipartite_graph_base(static_cast<unipartite_graph_base const&>(rhs))
      , base(static_cast<base const&>(rhs), hpx::take_ref::yes) {}
  ...
  public:
    ...
    partitioned_index_adjacency(size_t N, size_t M, Vector&& index_sizes,
                                Vector&& to_be_index_sizes, char const* name = "pg")
      requires(std::is_same_v<unipartite_graph_base, unipartite_graph_base>)
      : unipartite_graph_base(N)
      , base(N, M, std::forward<Vector>(index_sizes), std::forward<Vector>(to_be_index_sizes),
             name) {}

    // Create reference to partitioned_index_adjacency
    partitioned_index_adjacency ref() const {
      return partitioned_index_adjacency(*this, hpx::take_ref::yes);
    }
