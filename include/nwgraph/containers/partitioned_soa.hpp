...
namespace nw::graph {
  ...
  public:
    ...
    void register_as(char const* name, hpx::util::index_pack<Is...>, std::tuple<Ts...>& ts) {
      (std::get<Is>(ts).register_as(hpx::launch::sync, generate_name<Is>(name)), ...);
    }

    template <typename Vector>
    explicit partitioned_struct_of_arrays(size_t M, Vector&& sizes, char const* name)
      : base(hpx::partitioned_vector<Attributes>(
          M,
          hpx::explicit_container_layout(std::forward<Vector>(sizes),
                                         hpx::find_all_localities()))...) {

      using pack = hpx::util::make_index_pack_t<sizeof...(Attributes)>;
      register_as(name, pack{}, static_cast<base&>(*this));
    }

    // shallow copy constructor, shallow-copies partitioned vectors
    partitioned_struct_of_arrays(partitioned_struct_of_arrays const& rhs, hpx::take_ref)
      : base(std::apply([](auto&&... args) { return std::make_tuple(args.ref()...); },
