...
namespace nw::graph {
  ...
  public: // fixme
    ...
    template <typename Vector>
    partitioned_indexed_struct_of_arrays(size_t N, size_t M, Vector&& index_sizes,
                                         Vector&& to_be_index_sizes, char const* name)
      : N_(N)
      , indices_(N + 1,
                 hpx::explicit_container_layout(
                   increment_last_partition(std::forward<Vector>(index_sizes)),
                   hpx::find_all_localities()))
      , to_be_indexed_(M, std::forward<Vector>(to_be_index_sizes), name) {
    ...
    partitioned_indexed_struct_of_arrays(partitioned_indexed_struct_of_arrays const& rhs,
                                         hpx::take_ref)
      : N_(rhs.N_)
      , indices_(rhs.indices_.ref())
      , to_be_indexed_(rhs.to_be_indexed_, hpx::take_ref::yes)
    {}
