...
static constexpr char USAGE[] =
  ...
  Usage:
      dtc.exe (-h | --help)
      dtc.exe -f FILE... [--version ID...] [-n NUM] [--lower | --upper] [--relabel] [--heuristic] [--log FILE] [--log-header] [--format FORMAT] [--partitions PARTS] [--batchsize SIZE] [-dvV] [THREADS...]
...
void run_bench(int argc, char* argv[]) {
  ...
  for (auto&& file : files) {
    ...
    //  auto [indices_i, to_be_indexed_i] =
    //    deserialize_adjacency_graph_segment(file, num_partitions, i);
    //  indices.push_back(std::move(indices_i));
    //  to_be_indexed.push_back(std::move(to_be_indexed_i));
    //}

    std::cout << "loading partitioned adjacency\n";
    auto cel_a = load_partitioned_adjacency(file, num_partitions);
